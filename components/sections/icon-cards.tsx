    ...
    >
      {items.map((item, index) => (
        <Reveal
          key={item.title}
          delay={(index % 3) * 70}
          className="flex flex-col items-center rounded-[1.375rem] border-2 border-navy-600 bg-white/90 p-6 text-center backdrop-blur-sm transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1 hover:border-navy-900 hover:shadow-lift sm:rounded-[1.625rem] sm:p-7"
        >
          {/* The icon sits in a white tile of its own, so it reads as a mark
              rather than as loose line-work on the card. */}
          <span className="inline-flex h-14 w-14 items-center justify-center rounded-2xl border border-navy-200 bg-white text-navy-600 shadow-soft [&>svg]:h-7 [&>svg]:w-7">
            {item.icon}
          </span>
          <dt className="mt-4 font-display text-[1.0625rem] leading-snug text-navy-900 sm:text-[1.1875rem]">
            {item.title}
          </dt>
          {item.body && (
            <dd className="mt-2.5 text-[0.875rem] leading-relaxed text-slate-body">
              {item.body}
            </dd>
          )}
        </Reveal>
